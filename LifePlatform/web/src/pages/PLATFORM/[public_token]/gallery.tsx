...
import {
  ...
  useHerinneringen_GallerijQuery
...
const Gallery: React.FC<{}> = ({}) => {
  ...
  const [modalState,setmodalState] = useState('media');
  ...
  const { data, error, loading, fetchMore, variables } = useHerinneringen_GallerijQuery({
    variables: {
      limit: 25,
      cursor: null,      
  ...
  return (
    ...
    {!data && loading ? (
          ...
          <>
           {data!.herinneringen_gallerij.mediaHerinneringen.length===0 ? 
          ...
          :
           <div className="gallery_media_container">
            <div className="masonry_gallerygrid">
              {data!.herinneringen_gallerij.mediaHerinneringen.map((mediaHerr: any) =>
                !mediaHerr ? null : 
                <>
        ...
        {data && data.herinneringen_gallerij.hasMore ? (
          ...
          <button   onClick={() => {
                fetchMore({
                  variables: {
                    ...
                    cursor:
                      data.herinneringen_gallerij.mediaHerinneringen[
                        data.herinneringen_gallerij.mediaHerinneringen.length - 1
                      ].createdAt,
                  },
                });
    ...
    :null}
      ...
      {Error_value ?            
          <ErrorModal  active={Error_value} setactive={setError_value} error_type={'max_memories'}  link={`/PLATFORM/${public_token}/settings`}/>
              :null}
    </>
  );
};
