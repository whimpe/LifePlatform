...
const styles = (theme_aeterna:any) => ({
  ...
  interface StackGridMemoryElementProps {
    herinnering: Herinnering;
  }
  
  const StackGridMemoryElement: React.FC<StackGridMemoryElementProps> = ({
    herinnering: herr,
  }) => {
  ...
  <>
     <div key={herr.id} className='stackgrid_memory_box' >   
            ...
            <div className='memory_textbox_view'>         
              <div className='memory_textbox_title'> {herr.title}</div> 
               <div className='memory_subtext'>{herr?.text ? `${herr?.text?.slice(0,300)}...` :''}</div>
        

              <div className="memory_item_btn_container">
                <div className="memory_item_btn" onClick={()=>router.push(`/PLATFORM/${public_token}/memory/${herr.id}`)}>Bekijk herinnering</div>  
  ...
  export default WithApollo({ ssr: false })(StackGridMemoryElement);
