...
import { BsEnvelope, BsFillLockFill, BsImages } from "react-icons/bs";
...
const styles = (theme_aeterna:any) => ({
  ...
  interface DEMO_StackGridMemoryElementProps {
    herinnering: Herinnering;
  }
  
  const DEMO_StackGridMemoryElement: React.FC<DEMO_StackGridMemoryElementProps> = ({
    herinnering: herr,
  }) => {
  ...
  <>
     <div key={herr.id} className='stackgrid_memory_box' >   
            ...
            {!herr.media[0] ? null : (
                // <div className="stackgrid_memory_img_box">           

                <MediaDisplay awsUrl={herr.media[herr.media.length-1].urlFile} width={'100%'} type={herr.media[herr.media.length-1].mediaType}   />
                // </div> 
...
{/* 
            ...
            <div className='memory_textpreview_box'>
              ...
              {/* {!herr.datumVanHerinnering ? null: <div className="memory_author">{formatDate(herr.datumVanHerinnering)}</div>} */}
              <div className="memory_item_btn_container">
                <div className="memory_item_btn" onClick={()=>router.push(`/VOORBEELD/${public_token}/memory/${herr.id}`)}>Bekijk het Verhaal</div>   {/*  <IoEyeOutline className='memory_item_btn_icon'/> */}
              </div>
  ...
  export default WithApollo({ ssr: false })(DEMO_StackGridMemoryElement);
