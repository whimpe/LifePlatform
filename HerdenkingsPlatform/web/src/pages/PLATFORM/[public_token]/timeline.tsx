...
import { useHerdenkingspaginaQuery, useHerinneringenByDateQuery, useHerinneringenQuery, useHerinneringQuery, useMeQuery } from "../../../generated/graphql";
...
// const years:Set<String> = [];    //all the unique years
// const years_index:Set<number> = []; //all the indexes of the unique years
...
//DATA IMPORT
  ...
  const { data:hpdata, error:herinneringenbydateERROR, loading:herinneringenByDateLoading } = useHerinneringenByDateQuery({ variables: { paginaId: public_token, }, notifyOnNetworkStatusChange: true, });
...
async function zoom_in(herr_id:any,has_image:boolean,type:string) {
  set_memory_id(herr_id);
  if(has_image && type==='image' ){
    document.getElementById(herr_id)!.className = 'zoom_in_animation';
    await delay(2000);
  } 
  router.push(`/PLATFORM/${public_token}/memory/${herr_id}`)
}
...
for (let i = 0; i < hpdata!.herinneringenByDate.length ; i++) {
  if (!years.includes(get_year_db(hpdata?.herinneringenByDate[i].datumVanHerinnering))){
    years.push(get_year_db((hpdata?.herinneringenByDate[i].datumVanHerinnering)));
    years_index.push(i);
  }    
}
  ...
  return (
    ...
    { hpdata?.herinneringenByDate.length===0 ? null:
      <> 
        ...
        <div className="collage_btn_box" onClick={(e:any)=>{router.push(`/PLATFORM/${herdenkingspagina?.herdenkingspagina?.id}/memories`)}}> 
        ...
        <CreateContent  type={'timeline'}  name_of_page={''} modalClick={modalClick} setmodalClick={setmodalClick} modalState={modalState} setmodalState={setmodalState} />
            ...
            { hpdata?.herinneringenByDate.length===0 ?                    
              <>
                ...
                {hpdata?.herinneringenByDate.map((herr: any,herr_index:any) =>(
                  <>
                  <SwiperSlide>                     
                              {herr.media.length>0 ? 
                              <>
                                  <div className="media_timeline_wrapper" id='media_timeline_wrapper'>
                                      <a className='timeline_link_memory' onClick={(e:any)=>{zoom_in(herr.id ,true,herr.media[0].mediaType)}} >
                                  ...
                                  <div className="media_timeline_text_box">
                                      ...
                                      <div className="media_timeline_btn_box" onClick={(e:any)=>{zoom_in(herr.id ,true,herr.media[0].mediaType)}}><div className='media_timeline_btn'>BEKIJK HERINNERING</div> </div>   
                                  </div>
                              </>
                              :   
                              ...
                              <div className="media_timeline_text_only_box">
                                ...
                                <div className="media_timeline_btn_box" onClick={()=>{zoom_in(herr.id,false,'false')}}><div className='media_timeline_btn'>BEKIJK HERINNERING</div> </div>   
                              </div>
