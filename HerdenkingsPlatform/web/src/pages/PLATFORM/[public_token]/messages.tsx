...
import { CreateMessage_Modal } from "../../../components/modal_components/CreateMessage";
import ErrorModal from "../../../components/modal_components/ErrorModal";
import { AMOUNT_OF_MESSAGES_PLAN } from "../../../constants";
import {
...
const Messages: React.FC<{}> = ({}) => {
  ...
  const [message_obj, setmessage_obj] = useState({
    id: null,
    text: "",
    createdAt: null,
  ...
  const {
    data: own_messages_data,
    error: own_messages_error,
    loading: own_messages_loading,
  } = useOwnMessagesQuery({
    variables: {
      limit: 1000,
      cursor: null,
  ...
  if (hploading || messages_loading || own_messages_loading) {
  ...
  const check_allowed = (data: any) => {
    if (
      data.herdenkingspagina.number_of_messages >=
      AMOUNT_OF_MESSAGES_PLAN[data.herdenkingspagina.Payment_plan]
    ) {
      setError_value(true);
    } else {
      setmodalClick(true);
  ...
  var share_options = [
    ...
    "",
    "Publieke Berichten",
    "Steunberichten aan intieme kring",
    "Berichten met met beheerders herdenkingsplaats",
    `PrivéBerichten tussen ${herdenkingspagina?.herdenkingspagina?.name_of_page} en mezelf `,
  ];
    ...
    <>
      ...
      <Layout>
        <div className="messages_btn_container">
          ...
          >
            {" "}
            <GrGroup className="message_icon_box" />
          ...
          >
            {" "}
            <BiBookHeart className="message_icon_box" />
            intieme kring
          </button>
          {herdenkingspagina?.herdenkingspagina?.DoD ? (
            <button
              className="message_btn_box"
              onClick={(e: any) => {
                setContentType(5);
            ...
            >
              {" "}
              <AiOutlineCloud className="message_icon_box" />
              Bericht aan de overkant
            </button>
          ) : null}
        </div>
        ...
        <div className="messages_content_container">
          <div className="messages_img_box">
            <div className="messages_container_box">
              <img
                className="messages_img_view"
                src={herdenkingspagina?.herdenkingspagina?.mediaUrl}
              />
              <div className="messages_img_title">
                {" "}
                {herdenkingspagina?.herdenkingspagina?.name_of_page}
              </div>
            </div>
            {messages_data?.messages.berichten.length !== 0 ? (
              <div className="messages_img_subtitle">
                Laatste boodschap op{" "}
                {formatDate_text(
                  messages_data?.messages.berichten[
                    messages_data?.messages.berichten.length - 1
                  ].createdAt
                )}
              </div>
            ) : null}
          ...
          <div className="messages_content_box">
            <div className="messages_content_title">
              {share_options[ContentType]}
            </div>

            <div className="messages_content_container_box">
              {messages_data?.messages.berichten.map((message: any) =>
                !message ? null : (
                  <div key={message.id}>
                    {message.status === ContentType ? (
                      <div className="message_content_item">
                            ...
                            <div className="message_media_containter">
                              {message.media.map((media_file: any) => (
                                <div key={media_file.id}>
                                  <div
                                    className="message_media_box"
                                    onClick={(e: any) =>
                                      OpenLightBox_img(media_file)
                                    }
                                  >
                                    <MediaDisplay
                                      awsUrl={media_file.urlFile}
                                      type={media_file.mediaType}
                                      designclass="message_media_item"
                                    />
                                  </div>
                                </div>
                              ))}
                        ...
                        <div
                          ...
                          onClick={(e: any) => OpenLightBox_text(message)}
                        >
                          ...
                          <div className="message_content_info_container">
                            ...
                            <div className="message_item_date">
                              {formatDate_text(message.createdAt)}
                            </div>
                          </div>
                        </div>
                      </div>
              ...
              {own_messages_data?.ownMessages.berichten.map((message: any) =>
                !message ? null : (
                  <div key={message.id}>
                    {message.status === ContentType ? (
                      <div className="message_content_item">
        ...
        {modalClick ? (
          <>
            <div className="message_creator_container_big">
              <div className="droplet_message_btn_icon_container">
                <GrClose
                  className="droplet_message_btn_icon"
                  onClick={(e: any) => {
                    setmodalClick(false);
                  }}
                />{" "}
              ...
              <CreateMessage_Modal
                ...
                has_passed_away={!(herdenkingspagina?.herdenkingspagina?.DoD == undefined)}
              />
            </div>
          </>
        ) : (
          <>
            <div
              className="gradient_message_creator_container"
              onClick={(e: any) => {
                check_allowed(herdenkingspagina);
              }}
            >
              <div className="droplet_message_btn_icon_container">
                <IoMdAddCircleOutline className="droplet_message_btn_icon" />{" "}
              </div>
              <div className="droplet_message_text">
            ...
            <div className="lightbox2-content">
              <div key={message_obj.id} className="stackgrid_condolence_box">
                ...
                <div className="condolence_textpreview_box">
                    <div className="lightbox_condolence_text">
                      {message_obj.text}
                    </div>
                    <div className="lightbox_condolence_box_author">
                      {" "}
                      {message_obj.creator.username}
                    </div>
                    <div className="lightbox_condolence_box_date">
                      {" "}
                      {formatDate_text(message_obj.createdAt)}{" "}
                    </div>
                </div>
              </div>
            </div>
            ...
            <div key={media_object.urlFile}>
              <div className="gallery_item_view_box">
                ...
                <MediaDisplay
                  ...
                  designclass="lightbox2_gallery_item"
                />
                <div
                  onClick={(e: any) => {
                    setLightboxImgClick(false);
                  }}
                  className="lightbox2_gallery_elem_btn"
                >
                  terugkeren
                </div>
              </div>
        ...
        {Error_value ? (
          <ErrorModal
            ...
            error_type={"max_messages"}
            link={`/PLATFORM/${public_token}/settings`}
          />
        ) : null}
      </Layout>
    </>
